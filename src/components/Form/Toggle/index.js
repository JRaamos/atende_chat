...
export const Toggle = ({ checked, onChange, label, title, secondary }) => {
    ...
    return (
        <>
            <ThemedComponent>
                <FormGroup className="noPadding">
                    {title ? <FormLabel component="legend"
                        color={secondary ? 'secondary' : 'primary'} >{title}</FormLabel> : null}
                    <StyledFormControlLabel className="noPadding" control={<MaterialSwitch checked={checked} onChange={action} color={secondary ? 'secondary' : 'primary'} />}
                        label={label} />
                </FormGroup>
            </ThemedComponent>
